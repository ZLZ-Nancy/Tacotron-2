...
def build_from_path(hparams, input_dirs, mel_dir, linear_dir, wav_dir, n_jobs=12, tqdm=lambda x: x):
	...
	for input_dir in input_dirs:
		...
		# 		index += 1
		allfiles = os.path.join(input_dir, 'ProsodyLabeling')
		with open(allfiles + '/000001-010000.txt') as f:
			files = f.readlines()
			files = zip(*[iter(files)] * 2)
			files = [i[0] + i[1] for i in files]
			print(files[:10])
			for line in files:
				parts = line.strip().split('\t')
				wav_path = input_dir + '/Wave/' + parts[0] + '.wav'
				text = parts[2]
				basename = parts[0]
